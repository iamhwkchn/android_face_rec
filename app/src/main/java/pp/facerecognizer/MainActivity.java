...
public class MainActivity extends CameraActivity implements OnImageAvailableListener {
    ...
    private static final Size DESIRED_PREVIEW_SIZE = new Size(120, 120);
        ...
        runInBackground(
                () -> {
                    ...
                    if(!mappedRecognitions.isEmpty()) {
                        LOGGER.d("IMPORTANT MESSAGE THAT " + mappedRecognitions.get(0).getId() + "  " + mappedRecognitions.get(0).getConfidence());
                        if (mappedRecognitions.get(0).getTitle().equals("Donald Trump") && mappedRecognitions.get(0).getConfidence() >= 0.6) {
                            Intent i = new Intent(this, sample.class);
                            startActivity(i);
                        } else {
                            LOGGER.i("NO NOT IDENTIFIED");
                        }
                    }
                    ...
                    int cnt = 0;
                    for(Recognizer.Recognition result  : mappedRecognitions){
                        LOGGER.i(result.getTitle() + "    "+cnt++);
                    }
                });
